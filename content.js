...
// Content script - injects the page script into the page context
(function() {
  ...
  function injectScript(src) {
    ...
    // Remove the script element once it's loaded
    script.onload = function() {
      script.remove();
    };
  }

  // Use the external page-script.js file
  const scriptFile = "page-script.js";
  
  /* No longer using inline script injection
  const pageScript = `
  ...
  */
  ...
  // Function to inject the script when the page is ready
  function injectPageScript() {
    console.log("Preparing to inject page script");
    injectScript(scriptFile);
  }
  
  // Inject as early as possible for document_start
  if (document.readyState === 'loading') {
    console.log("Document still loading, waiting for DOMContentLoaded");
    document.addEventListener('DOMContentLoaded', function() {
      console.log("DOMContentLoaded fired, injecting script");
      injectPageScript();
    });
  } else {
    console.log("Document already loaded, injecting script immediately");
    injectPageScript();
  }
  
  // Also try again after a delay as a fallback
  setTimeout(function() {
    console.log("Fallback injection after 1 second");
    if (!document.getElementById('memrise-controls-v2')) {
      console.log("UI not found, attempting injection again");
      injectPageScript();
    } else {
      console.log("UI already exists, skipping fallback injection");
    }
  }, 1000);
  
  console.log("Memrise Speed Controller: Content script setup complete");
})();
